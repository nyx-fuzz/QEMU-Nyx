...
static bool append_page(page_cache_t *self, uint64_t page, uint64_t cr3)
{
    ...
    // if(!dump_page_cr3_snapshot(page, self->page_data+(PAGE_SIZE*self->num_pages), self->cpu,
    // GET_GLOBAL_STATE()->pt_c3_filter)){ 	if(!dump_page_cr3_snapshot(page,
    // self->page_data+(PAGE_SIZE*self->num_pages), self->cpu, GET_GLOBAL_STATE()->parent_cr3)){
    ...
    /*
     *  if(!dump_page_cr3_ht(page, self->page_data+(PAGE_SIZE*self->num_pages),
     * self->cpu, GET_GLOBAL_STATE()->parent_cr3)){ fprintf(stderr, "FAILED DUMP
     * PROCESS of PAGE %lx\n", page); assert(false);
     *      }
     */
     ...
     * //fast_loadvm();
     * if(cr3){
     *  dump_page_cr3_ht(page, self->page_data+(PAGE_SIZE*self->num_pages), self->cpu,
     * GET_GLOBAL_STATE()->parent_cr3); //self->cpu->parent_cr3);
     *  //assert(dump_page_cr3_snapshot(page, self->page_data+(PAGE_SIZE*self->num_pages),
     * self->cpu, GET_GLOBAL_STATE()->parent_cr3)); //self->cpu->parent_cr3);
     *
     *  //read_virtual_memory_cr3(page, self->page_data+(PAGE_SIZE*self->num_pages), PAGE_SIZE,
     * self->cpu, self->cpu->parent_cr3);
     * }
     * else{
     *  dump_page_ht(page, self->page_data+(PAGE_SIZE*self->num_pages), self->cpu);
     *  //read_virtual_memory(page, self->page_data+(PAGE_SIZE*self->num_pages), PAGE_SIZE,
     * self->cpu);
     * }
     */
...
bool append_page(page_cache_t *self, uint64_t page, uint8_t *ptr)
{
    ...
    assert(write(self->fd_address_file, &page, PAGE_CACHE_ADDR_LINE_SIZE) ==
           PAGE_CACHE_ADDR_LINE_SIZE);
            ...
            *k = kh_get(PC_CACHE, self->lookup, page);
            ...
            if (append_page(self, page, cr3)) {
                *k = kh_put(PC_CACHE, self->lookup, page, &ret);
                 ...
                 * assert(write(self->fd_address_file, &new_page,
                 * PAGE_CACHE_ADDR_LINE_SIZE) == PAGE_CACHE_ADDR_LINE_SIZE); kh_value(self->lookup,
                 * *k) = UNMAPPED_PAGE; fprintf(stderr, "APPEND UNMAPPED PAGE
                 * %lx!\n", page);
                 */
            }
...
#else
    nyx_debug_p(PAGE_CACHE_PREFIX, "%s (%s - %s) WORD_WIDTH: %d", __func__, tmp1,
                tmp2, disassembler_word_width);
#endif
                                 ...
                                 disassembler_mode_t mode)
{
    ...
    *failed_page = 0xFFFFFFFFFFFFFFFFULL;
    ...
    if ((*address & 0xFFF) >= (0x1000 - 16)) {
        ...
        // (*address&0xFFF), (0x1000-16), 0xf-(0xfff-(*address&0xfff)));
        ...
        if (success == true) {
                                ...
                                &code_size, address, insn))
            {
                *failed_page = (*address + 0x1000) & 0xFFFFFFFFFFFFF000ULL;
                ...
                // if(*address != 0x555555554ffe && *address != 0x7ffff7478ffc && *address !=
                // 0x7ffff7820ff6 && *address != 0x7ffff7822ffa) 	abort();
                return false;
            }
            return true;
