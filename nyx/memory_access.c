...
bool remap_payload_slot(uint64_t phys_addr, uint32_t slot, CPUState *cpu)
{
    ...
    QLIST_FOREACH_RCU (block, &ram_list.blocks, next) {
        if (!memcmp(block->idstr, "pc.ram", 6)) {
            ...
            // printf("MMUNMAP: %d\n", munmap((void*)(((uint64_t)block->host) +
            // phys_addr), x86_64_PAGE_SIZE)); printf("MMAP: %p\n",
            // mmap((void*)(((uint64_t)block->host) + phys_addr), 0x1000, PROT_READ
                ...
                uint64_t cr3)
{
    ...
    QLIST_FOREACH_RCU (block, &ram_list.blocks, next) {
        if (!memcmp(block->idstr, "pc.ram", 6)) {
            ...
            // phys_addr), x86_64_PAGE_SIZE)); printf("MMAP: %p\n",
            // mmap((void*)(((uint64_t)block->host) + phys_addr), 0x1000, PROT_READ
            // | PROT_WRITE, MAP_SHARED | MAP_FIXED, fd, (i*x86_64_PAGE_SIZE)));
...
bool remap_payload_buffer(uint64_t virt_guest_addr, CPUState *cpu)
{
         ...
         i < (GET_GLOBAL_STATE()->shared_payload_buffer_size / x86_64_PAGE_SIZE); i++)
    {
        ...
        // hwaddr phys_addr = cpu_get_phys_page_attrs_debug(cpu,
        // ((virt_guest_addr+(i*x86_64_PAGE_SIZE)) & x86_64_PAGE_MASK), &attrs);
        uint64_t phys_addr =
        ...
        QLIST_FOREACH_RCU (block, &ram_list.blocks, next) {
            if (!memcmp(block->idstr, "pc.ram", 6)) {
                ...
                // printf("MMAP: %lx\n", mmap((void*)(((uint64_t)block->host) +
                // phys_addr), 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED |
                // MAP_FIXED, shared_payload_buffer_fd, (i*x86_64_PAGE_SIZE)));
    ...
    while (amount_copied < size) {
        ...
        phys_addr = (hwaddr)get_paging_phys_addr(cpu, env->cr[3], address) &
                    0xFFFFFFFFFFFFF000ULL; // != 0xFFFFFFFFFFFFFFFFULL)
                                           // nyx_debug_p(MEM_PREFIX, "TRANSLATE: %lx
                                           // -> %lx == %lx", address, phys_addr, phys_addr_2);
